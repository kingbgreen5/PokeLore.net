...
} from "../lib/sizeComparisonCharacters.js";
...
function SizeComparison({
  ...
  ] = useState(initialCorrections.sprites ?? {});
  const [
    baseCharacterCorrectionsById,
    setBaseCharacterCorrectionsById
  ] = useState(initialCorrections.comparisonCharacters ?? {});
  const [
  ...
  ] = useState(DEFAULT_SIZE_COMPARISON_CHARACTER_ID);
  const [pokemonIndex, setPokemonIndex] =
    useState([]);
  useEffect(() => {
    setLocalCorrectionsById(readLocalCorrections());
    setLocalCharacterCorrectionsById(readLocalCharacterCorrections());
    setComparisonCharacterId(readComparisonCharacterId());
  }, []);
  ...
  function ChartFrame({
    ...
    return (
      <section
        className={`size-chart-frame ${scrollable ? 'mobile' : 'desktop'}`}
        style={{
          // border:
          //   "1px solid #d5dce5",
          // borderRadius: "18px",
